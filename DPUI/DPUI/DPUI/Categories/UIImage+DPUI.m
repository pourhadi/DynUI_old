...
+ (UIImage*)imageNamed:(NSString*)name withStyle:(NSString*)styleName
{
    ...
    DPUIBackgroundStyle *background = style.overlay;
    if (background) {
        
        DPUIShadowStyle *innerShadow = style.innerShadow;
        ...
        if (background.colors.count == 1) {
            if (innerShadow && innerShadow.color) {
                ...
                image = [UIImage tintImage:image withColor:[UIImage uiColorToCIColor:background.colors[0]]];
            }
        } else {
        ...
        if (style.outerShadow) {
            ...
            DPUIShadowStyle *outerShadow = style.outerShadow;
