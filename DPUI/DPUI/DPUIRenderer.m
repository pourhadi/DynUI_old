...
+ (NSOperationQueue *)drawQueue {
    NSOperationQueue *queue = objc_getAssociatedObject(self, kDPUIDrawQueueKey);
    if (!queue) {
        queue = [[NSOperationQueue alloc] init];
        objc_setAssociatedObject(self, kDPUIDrawQueueKey, queue, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return queue;
}
...
+ (void)renderNavigationBar:(UINavigationBar *)navigationBar withStyleNamed:(NSString *)styleName {
	...
	if (navStyle.shadow) {
		...
		if (!(CGSizeEqualToSize(navStyle.shadow.offset, CGSizeZero) && navStyle.shadow.radius == 0) && color.alpha != 0 && navStyle.shadow.opacity != 0) {
			...
			[navigationBar setShadowImage:[UIImage blankOnePointImage].dpui_resizableImage];
		}
	}
...
+ (void)applyImage:(UIImage *)image fromStyle:(DPUIViewStyle *)style toButton:(UIButton *)button {
...
+ (void)applyImage:(UIImage *)image fromStyle:(DPUIViewStyle *)style toTableCell:(UITableViewCell *)tableCell {
